...
use crate::{block_cid::assert_block_cid, car_header::StreamEnd};
#[cfg(feature = "async")]
use crate::{car_block::decode_block_async, car_header::read_car_header_async};
#[cfg(feature = "sync")]
use crate::{car_block::decode_block_sync, car_header::read_car_header_sync};
    ...
    R: AsyncRead + Send + Unpin,
{
    ...
    pub async fn new(
        ...
        let header = read_car_header_async(r).await?;
        return Ok(CarReader {
            header,
            read_bytes: 0,
            validate_block_hash,
            decode_header_future: Some(Box::pin(decode_block_async(r))),
        });
    }
}

#[cfg(feature = "sync")]
pub struct SyncCarReader<'r, 'd: 'r> {
    pub header: CarHeader,
    read_bytes: usize,
    validate_block_hash: bool,
    reader: Option<&'r mut std::io::Cursor<&'d [u8]>>,
}

#[cfg(feature = "async")]
impl<'r, 'd> SyncCarReader<'r, 'd> {
    /// Decodes a CAR stream up to the header. Returns a `Stream` type that yields
    /// blocks. The CAR header is available in [`CarReader.header`].
    ///
    /// # Examples
    ...
    /// ```
    pub fn new(
        r: &'r mut std::io::Cursor<&'d [u8]>,
        validate_block_hash: bool,
    ) -> Result<SyncCarReader<'r, 'd>, CarDecodeError> {
        let header = read_car_header_sync(r)?;
        return Ok(SyncCarReader {
            header,
            read_bytes: 0,
            validate_block_hash,
            reader: Some(r),
...
#[cfg(feature = "sync")]
pub fn car_read_all_sync<'a>(
    r: &mut std::io::Cursor<&'a [u8]>,
    validate_block_hash: bool,
) -> Result<(Vec<(Cid, &'a [u8])>, CarHeader), CarDecodeError> {
    let mut decoder = SyncCarReader::new(r, validate_block_hash)?;
        ...
        match &mut me.decode_header_future {
            Some(decode_future) => match decode_future.as_mut().poll(cx) {
                ...
                Poll::Ready(Ok((r, cid, block, block_len))) => {
                    ...
                    me.decode_header_future = Some(Box::pin(decode_block_async(r)));
...
#[cfg(feature = "sync")]
impl<'r, 'd> std::iter::Iterator for SyncCarReader<'r, 'd> {
    type Item = Result<(Cid, &'d [u8]), CarDecodeError>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match decode_block_sync(reader) {
            Ok((cid, block, block_len)) => {
...
mod tests {
    ...
    #[test]
    #[cfg(feature = "async")]
    fn decode_carv1_helloworld_no_stream_async() {
    ...
    #[test]
    #[cfg(feature = "sync")]
    fn decode_carv1_helloworld_no_stream_sync() {
        executor::block_on(async {
            let car_filepath = "./tests/custom_fixtures/helloworld.car";
            let file = std::fs::read(car_filepath).unwrap();
            let mut cursor = std::io::Cursor::new(file.as_slice());
            let (blocks, header) = car_read_all_sync(&mut cursor, true).unwrap();

            let root_cid = Cid::from_str("QmUU2HcUBVSXkfWPUc3WUSeCMrWWeEJTuAgR9uyWBhh9Nf").unwrap();
            let root_block = hex::decode("0a110802120b68656c6c6f776f726c640a180b").unwrap();

            assert_eq!(blocks, vec!((root_cid, root_block.as_slice())));
            assert_eq!(header.version, CarVersion::V1);
            assert_eq!(header.roots, vec!(root_cid));
        })
    }
    ...
    #[test]
    #[cfg(feature = "async")]
    fn decode_carv1_helloworld_stream_async() {
        executor::block_on(async {
            let car_filepath = "./tests/custom_fixtures/helloworld.car";
            let mut file = async_std::fs::File::open(car_filepath).await.unwrap();
            let (blocks, header) = car_read_all(&mut file, true).await.unwrap();
    ...
    #[test]
    #[cfg(feature = "sync")]
    fn decode_carv1_helloworld_stream_sync() {
        executor::block_on(async {
            let car_filepath = "./tests/custom_fixtures/helloworld.car";
            let file = std::fs::read(car_filepath).unwrap();
            let mut cursor = std::io::Cursor::new(file.as_slice());
    ...
    #[test]
    #[cfg(feature = "async")]
    fn decode_carv1_basic_async() {
        executor::block_on(async {
            run_car_basic_test_async(
    ...
    #[test]
    #[cfg(feature = "sync")]
    fn decode_carv1_basic_sync() {
        run_car_basic_test_sync(
            "./tests/spec_fixtures/carv1-basic.car",
            "./tests/spec_fixtures/carv1-basic.json",
        )
    }
    ...
    #[cfg(feature = "async")]
    async fn run_car_basic_test_async(car_filepath: &str, car_json_expected: &str) {
    ...
    #[cfg(feature = "sync")]
    fn run_car_basic_test_sync(car_filepath: &str, car_json_expected: &str) {
        let expected_car = std::fs::read_to_string(car_json_expected).unwrap();
        let expected_car: ExpectedCarv1 = serde_json::from_str(&expected_car).unwrap();

        let file = std::fs::read(car_filepath).unwrap();
        let mut cursor = std::io::Cursor::new(file.as_slice());
        let mut streamer = SyncCarReader::new(&mut cursor, true).unwrap();
...
pub mod sync_util {
    pub trait CursorExt<'a> {
        fn get_slice(&mut self, len: usize) -> std::io::Result<&'a [u8]>;
        fn get_array<const LEN: usize>(&mut self) -> std::io::Result<&'a [u8; LEN]>;
    }
    ...
    impl<'a> CursorExt<'a> for std::io::Cursor<&'a [u8]> {
        fn get_slice(&mut self, len: usize) -> std::io::Result<&'a [u8]> {
            ...
            Ok(&self.get_ref()[position..position + len])
        }
        ...
        fn get_array<const LEN: usize>(&mut self) -> std::io::Result<&'a [u8; LEN]> {
            ...
            let position = self.position() as usize;
            self.seek(SeekFrom::Current(LEN as _))?;
            // SAFETY: previous calling seek guarantees converting to array is valid.
            Ok(unsafe { &*(&self.get_ref()[position] as *const u8 as *const [u8; LEN]) })
        }
    }
}
